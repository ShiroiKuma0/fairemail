...
* **Fixed: with link confirmation switched off, a link went to the system app chooser instead of the app chosen to open it — and that chooser could not even offer the chosen app.** Android leaves a handler that is not an approved link handler for the domain out of a general web-intent chooser, while an intent carrying `setPackage` reaches it directly, so a link cleaner picked in the dialog was unreachable by construction and nothing said why. The branch that runs when no dialog is shown is reachable for two different reasons and was treating them as one: either confirmation is off globally, or this one host is exempt from it. The two per-host instructions it consults are written only by the open-link dialog, and only as the other half of an exemption — its "do not ask again" checkbox together with one of its buttons. Both confirmation toggles then clear every per-host exemption but leave those instructions behind, so a host can keep an order to use the system chooser long after the exemption that recorded it is gone. With "Remove tracking parameters by default" on it is worse still, because the dialog hides the very checkboxes that could correct the stored choice, leaving it unreachable as well as stale. Both reads are now gated on the exemption actually being the reason for landing there; with confirmation off globally the only instruction the user has given is the global Open-with choice, which is what gets used. A stale key goes inert without being deleted, so per-host exemptions that are still in force keep working exactly as before

### 1.2333+008 — on FairEmail 1.2333

* **Fixed: a restore reported 0/0 for its whole length.** The bytes were counted correctly all along and thrown away at the seam. A progress broadcast carries `current`, `total` and `unit`, which is what the automation contract defines and what 応用管理 reads, plus an additive `bytes` extra for a caller that can show a byte figure beside an item count. The export path fills both, because it reports once per category as it goes; the import path filled only the extra, because nothing reports a category while an archive is still being spooled. So every line left reading zero of zero while tens of megabytes went past. The spool now reports through the fields that are actually read, with the total left at zero because a descriptor the caller opened has no length this app may ask for — it can be a pipe, and an invented total is worse than none
* **A restore now counts the whole way through, not only while the archive arrives.** Once the transfer ended the import took no progress sink at all, so from there to the terminal reply the only thing on the wire was the heartbeat repeating the last byte figure — and a repeated number cannot be told apart from a dead app, at exactly the point where the longer half of the work begins. It now reports the way the export already did: one step per finished category, a line per account as it is worked on, and a message counter through the mail store — the index records as they are read, then each message as its payload entries come past
* The handover from receiving to restoring gets a line of its own, sent **before** the read rather than after it: pulling the export JSON out of the archive and parsing it runs to seconds on a large backup, and that stretch used to be silent. Two of the counters have no denominator, and inventing one would be worse than leaving it out — the message index is JSON Lines read as a stream, so its length is known only at the last line, and counting it first would mean decompressing the whole index twice. The payload pass does have a real total, and it counts the numbering the archive itself carries rather than what this device decided to keep: a message dropped for a folder that is not here still went past
* **Fixed: three copy loops ended on a read of zero bytes rather than at end of stream.** A read of zero is permitted without meaning a stream is finished, so the natural test can take a pause for an ending and produce a short copy — an archive that stops early, or an attachment written out under its full name holding only part of its contents. Nothing throws, and the restore then reports success over a truncated file, which is the worst shape this defect can take. Neither stream is likely to do it in practice, but a correct restore should not rest on that

### 1.2333+006 — on FairEmail 1.2333

* **Fixed: a restore through the data door silently threw away the local mail store.** 応用管理 names no categories on an import, and the import read that as the *export* default set — the one the app falls back to when nobody has chosen, which leaves the mail out because it is large. So a 52 MB archive whose owner had deliberately asked for the mail came back as its eight small settings categories, and the door replied success. A restore that quietly returns less than was backed up is the same failure as a truncated archive, only quieter, and it cannot be told from a good restore until the day it matters. Naming nothing now means everything the archive carries; a caller that does name categories still narrows, never widens
* **Fixed: a backup described itself as holding less than it did.** The `describe` header listed only the default categories, so the local mail never appeared in the list a backup app stores beside the archive and shows you. Worse, that list is paired position by position with the per-category sizes measured inside the archive, so a list short by one did not merely lose a row — every size from the missing category onward was printed against the wrong label. The header now answers the whole catalogue, which is what it was always meant to be: what this app *can* export, not what one particular backup chose
* **Split the local mail into two categories — 「Local emails (bodies)」 and 「Email attachments」 — and both now start ticked.** Measured on a real backup, 1,088 message bodies came to 45.0 MB and 286 attachment payloads to 46.3 MB; attachments are already-compressed formats, so they are rather more than half the archive. Keeping them as one category meant the only way to leave the weight out was to leave the mail out, which is why the mail store shipped unticked and the ordinary backup of a mail client held no mail
* Attachment **rows** always travel with the message index whatever is chosen, so a bodies-only backup still knows which mail has attachments; only the payloads are optional. Attachments cannot travel alone, because without the index there is nothing to attach a payload to
* On import an attachment is marked **not available until its payload has actually been written**, with the size taken from the file on disk rather than from the record. The row carried "available" across from the export, which would have left attachments the app believed it held, could not open, and would never fetch again; anything that does not arrive is now downloaded from the server the first time it is opened
* The archive format moves to **2** because the manifest vocabulary gained a category id, while the oldest readable format stays at **1**. Every backup written before the split names only 「messages」 and has the payloads inside it, so payloads are taken unless an archive itself declares the category separately and the caller left it out — reading the selection literally there would have dropped every attachment in every existing backup, which is the failure the split exists to prevent
* **Fixed: the automation worker could die without telling anyone.** It took its wakelock in two statements above its own `try`, and an executor swallows whatever escapes a task into a result nobody reads — so a failure there ended the job with no reply, no progress and no log line, which from the calling app looks exactly like an app that accepted the job and then never ran it, and costs it its whole ten-minute silence budget before it gives up. Nothing now runs outside the catch, and a missing power manager is logged and worked around rather than fatal

### 1.2333+004 — on FairEmail 1.2333

* Implemented **v2 of the sister-app backup automation contract**: the headless export becomes a door a backup app can also pull this app **through**, taking it away with its data and putting it back on a phone that has just been wiped
* The **Automation export** switch now defaults **on** and the authorization token became optional (new `automation_require_token`, default off). A pasted secret cannot survive a wipe, and the restore case runs on a phone where nothing has been configured, so the token is now an extra a caller may be asked for rather than the gate
* A token sent to an app that does not require one is **ignored rather than refused**, so one switch being off can never turn into half a batch failing for no visible reason
* Both checks moved into a single `AutomationAuth.refuse`, which answers either nothing or the exact `ERROR:` line to reply with, so "automation disabled" and "bad token" cannot drift apart between entry points
* The Export/Import section gained a 「Use authorization token?」 switch, and the token row is now **hidden unless the token is actually being asked for** — a secret sitting under an off switch only invites pasting it somewhere it will do nothing
* New **data door**: a `ContentProvider` at `shiroikuma.fairemail.automation`, exported without a permission, answering `describe`, `export`, `import` and `cancel`. `import` exists **only** here and has no intent action, because an import overwrites every account and message while the export receiver is exported with no permission on it
* The caller is identified by the framework and checked three ways — an **exact package name** and never a prefix, the **uid the kernel reports**, and a **pinned SHA-256 signing certificate** — because a package name absent from a device is a name anyone can take, and a freshly wiped phone is exactly such a device
* The payload moves through a **`ParcelFileDescriptor` the caller supplies**, duplicated before it leaves the binder call and closed on every path out, so the backup app can rename, encrypt and checksum a file it owns end to end without this app ever writing into its directory
* Export and import run in a new `AutomationDataService` foreground service holding a partial wakelock: a backgrounded app writing for minutes is frozen mid-stream on EMUI, which yields a truncated archive underneath a success reply — the one failure indistinguishable from a good backup until the day it is restored
* An import is **spooled to a cache file rather than into memory**, because a restore reads the archive twice and this app ships the largest archive in the family
* **Fixed: a restore could silently lose itself.** The settings import wrote preferences with `apply()`, and the calling app force-stops this one the instant it hears success, so an asynchronous write still in flight was simply dropped and the restore reported success over data that was gone. It commits synchronously now, with a trailing flush for what the upgrade path queues behind our back
* **Fixed: automation replies were resting on a visibility grant nobody chose.** The manifest `<queries>` element named neither calling app, so every reply and progress broadcast aimed with `setPackage` was reaching them only through the implicit grant that arrives when another app interacts with you — working, but for a reason nobody picked, and one that lapses as soon as the interaction does. Both callers are now declared outright
* One progress sender now serves both doors, sending the correlation id as both `reply_id` and `job_id`, and it gained a **20-second heartbeat** that resends the last true line: this export reports once per category, so zipping a gigabyte-scale mail store went silent well past the two minutes after which a caller presumes the app dead
* The `describe` header states outright that the accounts category carries **every account and identity password — for an OAuth account, the live token — in plaintext inside an unencrypted archive**, so that fact is in front of you while you choose what to save rather than discovered later
* Capability discovery through three manifest `<meta-data>` entries readable **without waking the app**, so a frozen FairEmail can still be listed as backupable, and a header that reports no required runtime permissions — checked rather than assumed, since this app keeps its contacts in its own table and never writes the system contacts provider
