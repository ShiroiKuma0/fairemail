...
**📥 Latest release: [`1.2324+1`](https://github.com/ShiroiKuma0/fairemail/releases/latest)** — [all releases & APK downloads »](https://github.com/ShiroiKuma0/fairemail/releases)
...
The app id is renamed to `shiroikuma.fairemail` so it coexists with the official build, and it shows as **白い熊 FairEmail** on the home screen. Pro activation still works: the activation salt is pinned to the original package id, so a Pro unlock validates despite the rename. The fork is versioned as `1.<upstream>+<fork>` (e.g. `1.2324+1`) and the update check is taught to ignore the `+<fork>` suffix so it never falsely flags an "update" back to stock.
