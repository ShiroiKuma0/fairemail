...
| Versioning | `versionName` = `1.<upstream>+<fork>` (e.g. `1.2325+1`); `versionCode` = `<upstream> * 10000 + <fork>` (e.g. `23250001`). The `getForkBuild` literal in `app/build.gradle` is the fork build number — reset to **1** on every upstream rebase, **+1** on every subsequent local build. `getVersionCode()` keeps returning the bare upstream code (it feeds archivesName/changelog/signature paths; do not repurpose it). |
...
| Deployed APK names | `shiroikuma-fairemail_<versionName>_arm64-v8a.apk` (e.g. `shiroikuma-fairemail_1.2325+1_arm64-v8a.apk`), copied to `~/tmp/` AND pushed to `/sdcard/tmp/`. No datetime — matches the user's other sideloaded apps (denwa, futokxkb, simplex): `shiroikuma-<app>_<upstream>+<fork>_arm64-v8a.apk`. |
| Toolchain (tag 1.2325) | compileSdk=37, minSdk=23, targetSdk=37, NDK `27.3.13750724` (r27d), AGP/Gradle 9.x, Java toolchain 21 (upstream bumped 17→21 at 1.2317), kotlin-android plugin REMOVED. Host build JDK is OpenJDK 21 (`JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64`). SDK lives at `$HOME/android-sdk`; export `ANDROID_HOME`/`ANDROID_SDK_ROOT` for builds (set in the user's `.bashrc` but NOT in non-interactive shells, and no `local.properties` is committed). |
...
```
6077b5af62  skills: document the merged changelog workflow
64344168b4  Add a merged fork changelog to CHANGELOG.md
aced12199d  Add a fork README for the GitHub release
cdbc637acc  Custom theme: black dropdown spinners with a yellow border
0190c9ce79  skills: auto-deliver builds via /after-build (drop the transfer prompt)
0cf3f25d0c  Make compose field hints and separators legible and tunable
e061f187cc  docs: no attribution trailer in commits
57c8c56801  Refresh fork skill and CLAUDE docs for the 1.2325 rebase
35f5017abb  Custom launcher icon: black-yellow line-traced envelope
afe3d07f8e  Skill: never delete old APKs on the device when deploying
0ec01c8b17  Custom theme: yellow drawer border without dimming the content
d6eb442879  Custom theme: black push buttons with a yellow border
8a1f4fb726  Custom theme: black snackbars with a yellow border and yellow text
2b80eb16e7  Custom theme: black dialogs and popup menus with a yellow border
d1ff5abe03  Add upstream-new-version skill to drive the upstream rebase and build
02838243da  kxkb: document fork versioning and label, refresh skill for 1.2318
55c5afad33  Rename the sideloaded app label to 白い熊 FairEmail
1825620eec  Version the fork as the upstream version plus a local build number
d7cdbda1eb  kxkb: add agent config (CLAUDE.md + .claude/skills/)
f967af7ec8  Folded message list: optional two-line subject with trailing date
726753eda2  Custom fonts: expand to independent per-role selection across eight roles
754d3496df  Custom font picker: defer pref save so the activity recreates cleanly
77c2588f08  Add custom font and weight selection for message text
4544f0d9a8  Custom theme colours: route tvBody link colour through the override
8a34c5cae9  Custom theme colours: expand to 28 roles across 7 sections
e5a5358a0d  Pin Pro activation salt to original package id
ef06bcaa6e  Custom theme colours: refactor picker UI to be data-driven
4451b5aac6  Custom theme: hook XML-resolved colours via Activity-only Resources wrap
2412f1ec00  Custom theme: hook code-resolved colours to user prefs
a5da3f96dc  Custom theme: scaffold the customizable colour picker UI
a7a3bab54f  Custom theme: unread accent yellow, decouple sender colour, extend font size range
a3a59ef39b  Custom theme polish: swap subject/sender highlight target, never bold sender, add sender_italic toggle, accent yellow
180d7a1083  Add Custom theme: yellow-on-black with gold unread accent
f7ecb3540f  Customize github flavor for sideloaded shiroikuma.fairemail build
```
...
```bash
build_apk=app/build/outputs/apk/github/release/FairEmail-v1.2325a-github-release.apk   # archivesName uses the bare upstream code
version=1.2325+1                                            # versionName: 1.<upstream>+<fork> (bump <fork> each local build)
...
Periodically rebase `custom` onto a newer upstream tag (last done 1.2324 → 1.2325):
...
- **Versioning:** `getForkBuild` in `app/build.gradle` carries the fork build number. `versionCode = getVersionCode() * 10000 + getForkBuild()`; `versionName = "1." + getVersionCode() + "+" + getForkBuild()`. So upstream `2325` build `1` → versionName `1.2325+1`, versionCode `23250001`. Reset `getForkBuild` to 1 on each upstream rebase, +1 each subsequent local build. The scheme stays monotonic because the upstream code only increases. `getVersionCode()` is deliberately left returning the bare upstream code so archivesName, the `CHANGELOG.md` rename, the fdroid signature dirs, and `build_uuid` keep their upstream-keyed values.
...
- **Update-check robustness (`ActivityView`):** the github update checker compares `Double.parseDouble(info.tag_name)` against `Double.parseDouble(BuildConfig.VERSION_NAME)`. `1.2325+1` is not a parseable double, so the comparison now strips the `+<fork>` suffix before parsing; otherwise every check would log an exception and falsely report an update to M66B's upstream build.
