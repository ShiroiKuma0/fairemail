...
| Versioning | `versionName` = `1.<upstream>+<fork>`, fork number zero padded to three digits (e.g. `1.2329+001`); `versionCode` = `<upstream> * 10000 + <fork>` (e.g. `23290001`, unpadded arithmetic). The `getForkBuild` literal in `app/build.gradle` is the fork build number — reset to **1** on every upstream rebase, **+1** on every subsequent local build. `getVersionCode()` keeps returning the bare upstream code (it feeds archivesName/changelog/signature paths; do not repurpose it). |
...
| Deployed APK names | `shiroikuma-fairemail_<versionName>_arm64-v8a.apk` (e.g. `shiroikuma-fairemail_1.2329+001_arm64-v8a.apk`), copied to `~/tmp/` AND pushed to `/sdcard/tmp/`. No datetime — matches the user's other sideloaded apps (denwa, futokxkb, simplex): `shiroikuma-<app>_<upstream>+<fork>_arm64-v8a.apk`, fork number zero padded to three digits so the shared directories sort in build order. |
| Toolchain (tag 1.2329) | compileSdk=37, minSdk=23, targetSdk=37, NDK `27.3.13750724` (r27d), AGP/Gradle 9.x, Java toolchain 21 (upstream bumped 17→21 at 1.2317), kotlin-android plugin REMOVED. Host build JDK is OpenJDK 21 (`JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64`). SDK lives at `$HOME/android-sdk`; export `ANDROID_HOME`/`ANDROID_SDK_ROOT` for builds (set in the user's `.bashrc` but NOT in non-interactive shells, and no `local.properties` is committed). |
...
```
abdacd3896  Publish 1.2329+001: refresh changelog and README for the 1.2329 rebase
74c66d6a34  Reset the fork build number for the 1.2329 rebase
b13269c52d  Publish 1.2328+002: refresh changelog and README for the 1.2328 rebase
0be35d09e3  Bump the fork build number to 2
a381347266  Zero pad the fork build number in the version name
504da54117  Refresh fork skill and CLAUDE docs for the 1.2327 rebase
ad1b58969f  Publish 1.2327+1: refresh changelog and README for the 1.2327 rebase
7f092e79d0  Reset the fork build number for the 1.2327 rebase
fb12539e00  Publish 1.2326+8: refresh changelog and README for the cancel work
571e2ce626  Bump the fork build number to 8
f0f2cef553  Stop a running export, leaving no partial file behind
d94026cbec  Say which backup categories start ticked
73ef0a482a  Publish 1.2326+7: refresh changelog and README for the backup work
2e84ded20f  Bump the fork build number to 7
0b0b24aa79  Answer the sister app state export automation contract
fa2f15e769  Add the automation token gate and its Export Import rows
b8cad5d180  Export the local mail store as an opt-in backup category
29047d1377  Write backups as one ZIP named by the sister-app convention
56315e674b  Publish 1.2326+5: refresh changelog and README for the UI page
df9de75124  Bump the fork build number to 5
2253eeee5a  Custom theme: black toolbar overflow menu with a yellow border
e217b68f10  Open the UI page by long-pressing the toolbar hamburger buttons
1bf790d4b2  Add the 白い熊 FairEmail UI page with export/import replacing Backup
2efd288541  Publish 1.2326+1: refresh changelog and README for the 1.2326 rebase
b4b90735b3  Publish 1.2325+4: refresh README and changelog for the pro unlock
0ac61e193a  Hide the purchase section on the pro features screen
1d928290d6  Unlock all pro features unconditionally
764c4c24b2  skills: document the merged changelog workflow
4c3d392003  Add a merged fork changelog to CHANGELOG.md
0c231be91a  Add a fork README for the GitHub release
6693b55562  Custom theme: black dropdown spinners with a yellow border
2ae73e653f  skills: auto-deliver builds via /after-build (drop the transfer prompt)
42dee99732  Make compose field hints and separators legible and tunable
7e4aaec89e  docs: no attribution trailer in commits
927a362e9a  Refresh fork skill and CLAUDE docs for the 1.2326 rebase
d5ef03762b  Custom launcher icon: black-yellow line-traced envelope
9a134774c9  Skill: never delete old APKs on the device when deploying
d51acd984b  Custom theme: yellow drawer border without dimming the content
4851441361  Custom theme: black push buttons with a yellow border
57c492af93  Custom theme: black snackbars with a yellow border and yellow text
bddca34106  Custom theme: black dialogs and popup menus with a yellow border
5f45a9bbda  Add upstream-new-version skill to drive the upstream rebase and build
0c104492d9  kxkb: document fork versioning and label, refresh skill for 1.2318
5a5d451259  Rename the sideloaded app label to 白い熊 FairEmail
724bd2ead0  Version the fork as the upstream version plus a local build number
240783fbb8  kxkb: add agent config (CLAUDE.md + .claude/skills/)
59c76942a8  Folded message list: optional two-line subject with trailing date
10a4a68a30  Custom fonts: expand to independent per-role selection across eight roles
b54159fd63  Custom font picker: defer pref save so the activity recreates cleanly
99eac11c04  Add custom font and weight selection for message text
b6b68ba58e  Custom theme colours: route tvBody link colour through the override
4d169a16bc  Custom theme colours: expand to 28 roles across 7 sections
9a443d48ad  Custom theme colours: refactor picker UI to be data-driven
8d9d7d11e9  Custom theme: hook XML-resolved colours via Activity-only Resources wrap
643cec3169  Custom theme: hook code-resolved colours to user prefs
d0b714d4e4  Custom theme: scaffold the customizable colour picker UI
5825c6a96f  Custom theme: unread accent yellow, decouple sender colour, extend font size range
0bd06718e4  Custom theme polish: swap subject/sender highlight target, never bold sender, add sender_italic toggle, accent yellow
e537769d09  Add Custom theme: yellow-on-black with gold unread accent
d43727c4a7  Customize github flavor for sideloaded shiroikuma.fairemail build
```
...
```bash
build_apk=app/build/outputs/apk/github/release/FairEmail-v1.2329a-github-release.apk   # archivesName uses the bare upstream code
version=1.2329+001                                          # versionName: 1.<upstream>+<fork>, fork padded to 3 digits (bump <fork> each local build)
...
- **Update-check robustness (`ActivityView`):** the github update checker compares `Double.parseDouble(info.tag_name)` against `Double.parseDouble(BuildConfig.VERSION_NAME)`. `1.2329+001` is not a parseable double, so the comparison now strips the `+<fork>` suffix before parsing (the strip cuts at the `+`, so it is agnostic to the padding width); otherwise every check would log an exception and falsely report an update to M66B's upstream build.
