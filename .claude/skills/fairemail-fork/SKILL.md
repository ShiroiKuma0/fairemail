...
| Versioning | `versionName` = `1.<upstream>+<fork>`, fork number zero padded to three digits (e.g. `1.2332+001`); `versionCode` = `<upstream> * 10000 + <fork>` (e.g. `23320001`, unpadded arithmetic). The `getForkBuild` literal in `app/build.gradle` is the fork build number — reset to **1** on every upstream rebase, **+1** on every subsequent local build. `getVersionCode()` keeps returning the bare upstream code (it feeds archivesName/changelog/signature paths; do not repurpose it). |
...
| Deployed APK names | `shiroikuma-fairemail_<versionName>_arm64-v8a.apk` (e.g. `shiroikuma-fairemail_1.2332+001_arm64-v8a.apk`), copied to `~/tmp/` AND pushed to `/sdcard/tmp/`. No datetime — matches the user's other sideloaded apps (denwa, futokxkb, simplex): `shiroikuma-<app>_<upstream>+<fork>_arm64-v8a.apk`, fork number zero padded to three digits so the shared directories sort in build order. |
| Toolchain (tag 1.2332) | compileSdk=37, minSdk=23, targetSdk=37, NDK `27.3.13750724` (r27d), AGP/Gradle 9.x, Java toolchain 21 (upstream bumped 17→21 at 1.2317), kotlin-android plugin REMOVED. Host build JDK is OpenJDK 21 (`JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64`). SDK lives at `$HOME/android-sdk`; export `ANDROID_HOME`/`ANDROID_SDK_ROOT` for builds (set in the user's `.bashrc` but NOT in non-interactive shells, and no `local.properties` is committed). |
...
```
25f7986637  Publish 1.2331+001: refresh changelog and README for the 1.2331 rebase
91e0ac457a  Publish 1.2330+001: refresh changelog and README for the 1.2330 rebase
2441fd58f1  Publish 1.2329+001: refresh changelog and README for the 1.2329 rebase
303f651b4f  Reset the fork build number for the 1.2329 rebase
8957271bb3  Publish 1.2328+002: refresh changelog and README for the 1.2328 rebase
a95d4365ed  Bump the fork build number to 2
cd5592b03a  Zero pad the fork build number in the version name
79dc185253  Refresh fork skill and CLAUDE docs for the 1.2327 rebase
740bd25cbb  Publish 1.2327+1: refresh changelog and README for the 1.2327 rebase
ebdf3d2bdd  Reset the fork build number for the 1.2327 rebase
0112d46bd4  Publish 1.2326+8: refresh changelog and README for the cancel work
4f98a7622e  Bump the fork build number to 8
e4254ac225  Stop a running export, leaving no partial file behind
d970598af1  Say which backup categories start ticked
463771dedc  Publish 1.2326+7: refresh changelog and README for the backup work
af82a7081b  Bump the fork build number to 7
9cefc3beea  Answer the sister app state export automation contract
689df1205f  Add the automation token gate and its Export Import rows
09d7a9661a  Export the local mail store as an opt-in backup category
0a6fc8d837  Write backups as one ZIP named by the sister-app convention
049286a9bb  Publish 1.2326+5: refresh changelog and README for the UI page
7aee9b8dfb  Bump the fork build number to 5
53348288bd  Custom theme: black toolbar overflow menu with a yellow border
d094c92690  Open the UI page by long-pressing the toolbar hamburger buttons
2efda72c2b  Add the 白い熊 FairEmail UI page with export/import replacing Backup
fcc0320ca4  Publish 1.2326+1: refresh changelog and README for the 1.2326 rebase
93240faf72  Publish 1.2325+4: refresh README and changelog for the pro unlock
22aee26787  Hide the purchase section on the pro features screen
6944ad83cc  Unlock all pro features unconditionally
bc19630e67  skills: document the merged changelog workflow
ce64353149  Add a merged fork changelog to CHANGELOG.md
2c45e0ef66  Add a fork README for the GitHub release
62dbbcc8d6  Custom theme: black dropdown spinners with a yellow border
17244bb8ea  skills: auto-deliver builds via /after-build (drop the transfer prompt)
2cfbed4790  Make compose field hints and separators legible and tunable
9da8092a6e  docs: no attribution trailer in commits
b97496b128  Refresh fork skill and CLAUDE docs for the 1.2326 rebase
a62aa4828f  Custom launcher icon: black-yellow line-traced envelope
dc21019c96  Skill: never delete old APKs on the device when deploying
a9dc2ecfb5  Custom theme: yellow drawer border without dimming the content
3ba3dc6eac  Custom theme: black push buttons with a yellow border
c7101b4caa  Custom theme: black snackbars with a yellow border and yellow text
f0abbfb710  Custom theme: black dialogs and popup menus with a yellow border
5a152911dd  Add upstream-new-version skill to drive the upstream rebase and build
ffbad7880b  kxkb: document fork versioning and label, refresh skill for 1.2318
7692b7f880  Rename the sideloaded app label to 白い熊 FairEmail
0a04dd3258  Version the fork as the upstream version plus a local build number
d68e9806b3  kxkb: add agent config (CLAUDE.md + .claude/skills/)
719c6735c7  Folded message list: optional two-line subject with trailing date
796bb25e74  Custom fonts: expand to independent per-role selection across eight roles
d82998d920  Custom font picker: defer pref save so the activity recreates cleanly
070f347e17  Add custom font and weight selection for message text
9baa39e8b8  Custom theme colours: route tvBody link colour through the override
34631a5745  Custom theme colours: expand to 28 roles across 7 sections
63b2edb93e  Custom theme colours: refactor picker UI to be data-driven
da725e4633  Custom theme: hook XML-resolved colours via Activity-only Resources wrap
9a0eff53fa  Custom theme: hook code-resolved colours to user prefs
82e2d5cce6  Custom theme: scaffold the customizable colour picker UI
12919305fc  Custom theme: unread accent yellow, decouple sender colour, extend font size range
2617f2632d  Custom theme polish: swap subject/sender highlight target, never bold sender, add sender_italic toggle, accent yellow
ce27981826  Add Custom theme: yellow-on-black with gold unread accent
67347f3d61  Customize github flavor for sideloaded shiroikuma.fairemail build
```
...
```bash
build_apk=app/build/outputs/apk/github/release/FairEmail-v1.2332a-github-release.apk   # archivesName uses the bare upstream code
version=1.2332+001                                          # versionName: 1.<upstream>+<fork>, fork padded to 3 digits (bump <fork> each local build)
...
- **Update-check robustness (`ActivityView`):** the github update checker compares `Double.parseDouble(info.tag_name)` against `Double.parseDouble(BuildConfig.VERSION_NAME)`. `1.2332+001` is not a parseable double, so the comparison now strips the `+<fork>` suffix before parsing (the strip cuts at the `+`, so it is agnostic to the padding width); otherwise every check would log an exception and falsely report an update to M66B's upstream build.
