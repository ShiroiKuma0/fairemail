...
**Versioning**: before each build, set the fork build number in `app/build.gradle` (`getForkBuild`): bump it +1 on every local build, and reset it to **1** right after an upstream rebase. That yields versionName `1.<upstream>+<fork>` (e.g. `1.2324+1`) and versionCode `<upstream> * 10000 + <fork>` (e.g. `23240001`). Leave `getVersionCode()` returning the bare upstream code. See the fork skill for the full rationale.
