...
4. **Deploy hygiene**: copy to `~/tmp/shiroikuma-fairemail_<versionName>_arm64-v8a.apk` (versionName = `1.<upstream>+<fork>` with the fork number zero padded to three digits, e.g. `1.2332+001`) AND `adb push` to `/sdcard/tmp/`. **Never delete old APKs on the device** — leave every prior `/sdcard/tmp/shiroikuma-fairemail_*.apk` in place; the version in the filename keeps them apart, and the build-side mtime + integrity probe already guard against shipping a stale build.
...
**Versioning**: before each build, set the fork build number in `app/build.gradle` (`getForkBuild`): bump it +1 on every local build, and reset it to **1** right after an upstream rebase. That yields versionName `1.<upstream>+<fork>` with the fork number zero padded to three digits (e.g. `1.2332+001`) and versionCode `<upstream> * 10000 + <fork>` (e.g. `23320001`). The padding is cosmetic and lives only in the versionName, so build lists and APK file names sort in build order; the versionCode arithmetic is unchanged. Leave `getVersionCode()` returning the bare upstream code. See the fork skill for the full rationale.
